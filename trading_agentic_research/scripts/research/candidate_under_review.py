...
should refine that candidate without promoting it as the official parent.

Manual parent governance integration
------------------------------------
If state/current_parent.json or state/champion_runs.json contains
...
def _candidate_from_champion_state(state_dir: str | Path) -> str | None:
    ...
    for key in ("pending_parent_candidate_run_id", "baseline_candidate_run_id", "aggressive_champion_run_id"):
        value = current.get(key)
        if value and str(value) != str(current.get("current_parent_run_id")):
            return str(value)

    champion = load_champion_state(state_dir)
    for key in ("pending_parent_candidate_run_id", "best_champion_run_id", "baseline_candidate_run_id", "aggressive_champion_run_id"):
        value = champion.get(key)
        if value and str(value) != str(champion.get("current_parent_run_id")):
            return str(value)
