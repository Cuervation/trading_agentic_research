...
def compute_readiness(*, state_dir: str | Path = "state", registry_path: str | Path = "configs/strategy_registry.json") -> dict[str, Any]:
    ...
    add("parent_config_exists", bool(parent_config and Path(parent_config).exists()), 12, str(parent_config))
    ...
    hyp_bank = Path("bibliography/hypothesis_bank.jsonl")
    paper_ideas = Path("bibliography/paper_ideas.jsonl")
    ...
    feature_plan = Path("reports/feature_engineering_plan.md")
    ...
    if batch_state:
        add("last_batch_not_zero_iteration_success", int(batch_state.get("completed", 0) or 0) > 0 or batch_state.get("status") not in {"completed"}, 10, f"completed={batch_state.get('completed')}; status={batch_state.get('status')}")
    ...
    return {"score": score, "max_score": max_score, "readiness_pct": pct, "status": "ready" if pct >= 85 else "needs_attention", "failed_checks": [c for c in checks if not c["ok"]], "checks": checks}
...
def main() -> int:
    ...
    args = p.parse_args()
    print(json.dumps(compute_readiness(state_dir=args.state_dir, registry_path=args.registry), indent=2, ensure_ascii=False))
    return 0
