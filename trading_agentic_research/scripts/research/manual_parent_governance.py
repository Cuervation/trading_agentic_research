...
def enforce_manual_parent_governance(
    *,
    ...
    # Backward-compatible name used by some callers/tests.
    ...
    candidate_state = read_json(state_path / "candidate_under_review.json", {}) or {}
    existing_lock = read_json(lock_path, {}) or {}
    official = (
        official_parent
        or candidate_state.get("official_parent_run_id")
        or existing_lock.get("official_parent_run_id")
        or champion.get("official_parent_run_id")
        or champion.get("current_parent_run_id")
    )
    if not official:
        return {
            "status": "skipped",
    ...
    payload = {
        ...
        "pending_parent_candidate_run_id": pending,
        "parent_promotion_blocked": bool(pending) and str(pending) != str(official),
        "parent_updates_require_manual_approval": True,
...
enforce_manual_parent = enforce_manual_parent_governance
