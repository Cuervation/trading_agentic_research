...
from backtester.dd_first import (
    append_dd_first_summary,
    evaluate_dd_first_run,
    find_latest_run_for_strategy,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-month-margin", type=int, default=3)
    parser.add_argument("--min-year-margin", type=int, default=1)
...
def _resolve_parent_run(args: argparse.Namespace, run_dir: Path) -> Path | None:
    ...
    parent_strategy_id = (
        args.parent_strategy_id
        or manifest.get("parent_strategy_id")
        or _parent_strategy_id_from_config(manifest.get("strategy_config_path"))
    )
...
def _parent_strategy_id_from_config(config_path: str | None) -> str | None:
    if not config_path:
        return None
    ...
    if not path.exists():
        return None
    cfg = _read_json(path)
    return cfg.get("parent_strategy_id")
...
def main() -> int:
    ...
    audit = evaluate_dd_first_run(
        run_dir,
        parent_run_dir=parent_run,
        min_trades=args.min_trades,
        min_month_margin=args.min_month_margin,
        min_year_margin=args.min_year_margin,
    )

    audit_path = run_dir / "dd_first_audit.json"
    audit_path.write_text(json.dumps(audit, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
    output_path = args.output or str(Path(args.reports_dir) / "dd_first_summary.csv")
    append_dd_first_summary(output_path, audit["row"])
