...
def load_previous_rows(previous_report_dir: str | Path | None) -> list[dict[str, Any]]:
    ...
    candidates = [prev / "grid_results.csv", prev / "combined_grid_results.csv"]
...
def generate_reports(out: Path, state: dict[str, Any], previous_rows: list[dict[str, Any]] | None = None) -> None:
    ...
    baseline_rows = [r for r in state_rows if r.get("strategy_id") == BASELINE_ID]
...
"""
    ...
    try:
        ...
        excel_name = "dd_guard_grid_results_extended.xlsx" if previous_rows else "dd_guard_grid_results.xlsx"
...
def main() -> int:
    ...
    variants = generate_extended_variants(base_config, max_count) if args.previous_report_dir else generate_variants(base_config, max_count)
